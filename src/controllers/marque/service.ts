...
export const getAllService=async (data:any, persist:boolean=true)=>{
    const format=(data:any):Prisma.MarqueFindManyArgs=>{
        return data
    }
    try{
        if(persist){
            return await prisma.marque.findMany(format(data))
        }else{
            return format(data)
        }
    }catch(e){
...
export const getOneService=async (data:any, persist:boolean=true)=>{
    const format=(data:any):Prisma.MarqueFindUniqueArgs=>{
        return {
            where:{
                id:data.id
    ...
    try{
        if(persist){
            return await prisma.marque.findUnique(format(data))
        }else{
            return format(data)
        }
    }catch(e){
...
export const postService=async (data:any, persist:boolean=true)=>{
    const format=(data:any):Prisma.MarqueCreateArgs=>{
        return {
            data:{
                id:v4(),
                ...data
    ...
    try{
        if(persist){
            return await prisma.marque.create(format(data))
        }else{
            return format(data)
        }
    }catch(e){
...
export const updateService=async ({id, ...data}:any, persist:boolean=true)=>{
    const format=({id, ...data}:any):Prisma.MarqueUpdateArgs=>{
        return {
            data,
            where:{
                id
    ...
    try{
        if(persist){
            return await prisma.marque.update(format({id, ...data}))
        }else{
...
export const deleteService=async (data:any, persist:boolean=true)=>{
    const format=({id}:any):Prisma.MarqueDeleteArgs=>{
        return {
            where:{
                id
            }
    ...
    try{
        if(persist){
            return await prisma.marque.delete(format(data))
        }else{
            return format(data)
        }
    }catch(e){
