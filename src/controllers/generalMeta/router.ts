...
import { GeneralMetaInput } from "./input";
...
export const generalMetaRouter=express.Router()


generalMetaRouter.get('/', async(req:express.Request, res:express.Response)=>{
    try{
...
generalMetaRouter.get('/:id', async (req:express.Request, res:express.Response)=>{
...
generalMetaRouter.post('/',authOnlyAdmin, async (req:express.Request, res:express.Response)=>{
    const {data}=req.body
    if(data){
        const {_status=200, ...result}=await dtoResolver(data, {
            ...
            validateOnType: GeneralMetaInput,
            context:"create"
        })
        return res.status(_status).json({...result, status:_status})
    }else{
...
generalMetaRouter.patch('/:id',authOnlyAdmin, async (req:express.Request, res:express.Response)=>{
    let {data}=req.body
...
generalMetaRouter.delete('/:id',authOnlyAdmin, async (req:express.Request, res:express.Response)=>{
    const {id}=req.params
    if(id){
