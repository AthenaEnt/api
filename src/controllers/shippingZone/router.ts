...
import { ShippingZoneInput } from "./input";
...
export const shippingZoneRouter=express.Router()


shippingZoneRouter.get('/', async(req:express.Request, res:express.Response)=>{
    try{
...
shippingZoneRouter.get('/:id', async (req:express.Request, res:express.Response)=>{
...
shippingZoneRouter.post('/',authOnlyAdmin, async (req:express.Request, res:express.Response)=>{
    const {data}=req.body
    if(data){
        const {_status=200, ...result}=await dtoResolver(data, {
            ...
            validateOnType: ShippingZoneInput,
            context:"create"
        })
        return res.status(_status).json({...result, status:_status})
    }else{
...
shippingZoneRouter.patch('/:id',authOnlyAdmin, async (req:express.Request, res:express.Response)=>{
...
shippingZoneRouter.delete('/:id',authOnlyAdmin, async (req:express.Request, res:express.Response)=>{
    const {id}=req.params
    if(id){
